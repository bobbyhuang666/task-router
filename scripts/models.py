...
def call_ollama(prompt: str, model: Optional[str] = None, max_tokens: Optional[int] = None,
                with_logprobs: bool = False, think: Optional[bool] = None) -> dict[str, Any]:
    ...
    try:
        # 预估 prompt token 数，动态设置 num_ctx（避免大上下文浪费内存）
        prompt_chars = len(prompt)
        estimated_prompt_tokens = max(128, prompt_chars // 3)  # 粗略估算
        num_ctx = min(4096, max(1024, estimated_prompt_tokens + max_tokens + 128))
        ...
        if with_logprobs:
            body["logprobs"] = True
        # thinking 模型：默认关闭 thinking 以节省 token
        # 对不支持此参数的模型，Ollama 会自动忽略
