...
def _select_by_keywords(
    ...
    实验表明 CoD（Chain-of-Draft）全面优于 direct 和 cot：
    - 比 direct 快 42%、省 46% token、置信度高 21%
    因此 CoD 是中等复杂度任务的默认策略。
    """
    ...
    # 3. 需要深度推理的触发词 → CoT（仅限明确需要逐步推理的任务）
    deep_reasoning = ["推理", "论证", "证明", "推导", "假设"]
    deep_matches = [kw for kw in deep_reasoning if kw in action_lower]
    if deep_matches and complexity_score >= 4.0:
        return StrategyDecision(
            strategy=STRATEGY_COT,
            reason=f"需要深度推理 (触发词: {','.join(deep_matches[:3])})",
            complexity_score=complexity_score,
            token_budget_factor=TOKEN_BUDGET[STRATEGY_COT],
        )

    # 4. 中等复杂度（1.5-6.0）→ CoD（默认策略，实验验证最优）
    if 1.5 <= complexity_score < 6.0:
        return StrategyDecision(
            strategy=STRATEGY_COD,
            reason=f"中等复杂度，CoD 最优 (score={complexity_score:.1f})",
            complexity_score=complexity_score,
...
def _select_by_quantile(
    ...
    # ── 策略选择决策树（实验优化版：CoD 为默认策略）──

    # 1. 需要结构化输出 → structured
    if needs_structured and complexity_score >= 2.5:
        trigger = next((t for t in STRUCTURED_TRIGGERS if t in action_lower), "unknown")
        return StrategyDecision(
            strategy=STRATEGY_STRUCTURED,
    ...
    # 2. 高置信度（>=0.8）→ direct（模型非常确定，无需推理）
    if confidence >= 0.8 and qf.token_count > 0:
        return StrategyDecision(
            strategy=STRATEGY_DIRECT,
    ...
    # 4. 中等置信度或中等复杂度 → CoD（实验验证：比 direct 快42%、省46%token、置信度高21%）
    if (0.2 <= confidence < 0.8 and complexity_score < 5.0) or (qf.token_count == 0 and complexity_score < 5.0):
        return StrategyDecision(
            strategy=STRATEGY_COD,
            reason=f"CoD 最优 (conf={confidence:.2f}, score={complexity_score:.1f})",
            complexity_score=complexity_score,
            confidence_signal=confidence,
            token_budget_factor=TOKEN_BUDGET[STRATEGY_COD],
        )

    # 5. 低置信度 + 高复杂度 → CoT（需要深度推理）
    if confidence < 0.3 or complexity_score >= 5.0:
        return StrategyDecision(
            strategy=STRATEGY_COT,
            reason=f"需要深度推理 (conf={confidence:.2f}, score={complexity_score:.1f})",
            complexity_score=complexity_score,
            confidence_signal=confidence,
            token_budget_factor=TOKEN_BUDGET[STRATEGY_COT],
        )
...
class StrategyTracker:
    ...
    def get_success_rate(self, task_type: str, strategy: str) -> float:
        ...
        with self._lock:
            key = f"{task_type}:{strategy}"
            records = list(self._success_stats.get(key, []))
        if len(records) < 3:
            return -1.0
