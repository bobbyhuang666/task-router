...
class TokenQuantileFeatures:
    ...
    overall_confidence: float = 0.0    # 综合置信度（所有 token 的平均 margin）
    token_count: int = 0
...
def extract_quantile_features(logprobs: list[dict]) -> TokenQuantileFeatures:
    ...
    for i, lp in enumerate(logprobs):
        ...
        if not top:
            ...
            # 归一化到 [0, 1]：用 sigmoid 避免无界 margin 污染 calibeating 分箱
            margins.append(1.0 / (1.0 + math.exp(logp)))
            continue
        ...
        if total > 0:
            ...
            # 所有概率下溢到 0：最大不确定性（不是 0）
            entropies.append(math.log(max(1, len(probs))))
            margins.append(0.0)
            continue
    ...
    # 综合置信度：加权组合（实验优化：q50_margin 对某些模型偏低，需要补偿）
    overall_conf = (
        0.3 * q50_margin +
        0.3 * first_margin +
        0.2 * percentile(sorted_mar, 75) +
        0.2 * min(1.0, max(margins) if margins else 0.0)
    )
...
def quantiles_to_feature_vector(qf: TokenQuantileFeatures) -> list[float]:
    """将分位数特征转换为 7 维特征向量（不含 bias，由调用方添加）"""
    # 归一化到 [0, 1]
    ...
    def update(self, arm: str, features: list[float], reward: float) -> None:
        ...
        with self._lock:
            if len(features) != self.n_features:
                raise ValueError(
                    f"Feature dimension mismatch: expected {self.n_features}, got {len(features)}"
                )
            params = self._params[arm]
...
class BayesianConfidenceCalibrator:
    ...
    def get_stats(self) -> dict:
        """获取校准统计"""
        with self._lock:
            n_obs = self._params.get("n_obs", 0)
            return {
                "total": n_obs,
                "global_params": {k: round(v, 4) if isinstance(v, float) else v
                                  for k, v in self._params.items() if k != "n_obs"},
                "groups": {
...
class TQBCDecision:
    ...
    raw_confidence: float          # 校准前的原始置信度（用于 record_outcome 反馈）
    uncertainty: float
    thompson_arm: str
...
class ConfidenceGapTracker:
    ...
    def _load(self) -> None:
        path = os.path.join(self._cache_dir, "tqbc_gap_tracker.json")
        if os.path.exists(path):
            try:
                with open(path) as f:
                    data = json.load(f)
                self._correct_confs = data.get("correct", [])[-self.WINDOW:]
                self._incorrect_confs = data.get("incorrect", [])[-self.WINDOW:]
            except (json.JSONDecodeError, TypeError):
                pass
    ...
    def _save(self) -> None:
        ...
        try:
            with open(tmp_path, "w") as f:
                json.dump({
                    "correct": self._correct_confs[-self.WINDOW:],
                    "incorrect": self._incorrect_confs[-self.WINDOW:],
                }, f)
            os.replace(tmp_path, path)
        except OSError:
            pass
...
class TQBCRouter:
    ...
    COLD_START_MIN_OBS = 20  # 冷启动保护：校准样本不足时使用原始置信度
    ...
    def __init__(self, cache_dir: str):
        ...
        self.gap_tracker = ConfidenceGapTracker(cache_dir=cache_dir)
    ...
    def decide(
        ...
        # 冷启动保护：校准样本不足时直接使用原始置信度，不调用 calibrate
        n_obs = self.calibrator._params.get("n_obs", 0)
        group_n = self.calibrator._group_params.get(task_type, {}).get("n_obs", 0) if task_type else 0
        effective_n = max(n_obs, group_n)
        if effective_n < self.COLD_START_MIN_OBS:
            # 冷启动期间融合 q50_margin 和 overall 信号
            # q50_margin 可能偏低，用 overall_margin 补偿
            overall_margin = qf.overall_confidence if hasattr(qf, 'overall_confidence') else raw_conf
            calibrated_conf = 0.4 * raw_conf + 0.6 * overall_margin
            uncertainty = 0.5  # 高不确定性 → 保守决策
        else:
        ...
        # 冷启动期间更保守：降低升级阈值（= 提高升级门槛），减少误升级
        if effective_n < self.COLD_START_MIN_OBS:
            effective_threshold = min(0.25, effective_threshold)

        # 融合 Thompson Sampling 结果和校准置信度
        # 冷启动期间仅依赖置信度阈值，不使用 TS 决策（TS 数据不足不可靠）
        if effective_n < self.COLD_START_MIN_OBS:
            should_escalate = calibrated_conf < effective_threshold
        else:
            should_escalate = (
    ...
    def record_outcome(
        ...
        # 更新校准器（使用原始置信度，避免双重 Platt 变换）
        if not escalated:  # 只有未升级的结果用于校准
            self.calibrator.record(decision.raw_confidence, success, task_type)
